<?php
    ...
    if(!empty($shorten_url)){
        if(preg_match("/\//i", $shorten_url)){
            ...
            if($shortURL != ""){
                $sql = mysqli_query($conn, "SELECT shorten_url FROM allurl WHERE shorten_url = '{$shortURL}' && shorten_url != '{$hidden_url}'");
                if(mysqli_num_rows($sql) == 0){
                    $sql2 = mysqli_query($conn, "UPDATE allurl SET shorten_url = '{$shortURL}' WHERE shorten_url = '{$hidden_url}'");
                    if($sql2){
                        echo "success";
                    }else{
